...
the plugin validator. When runtime dependencies change, regenerate the hash-locked runtime file with
`uv export --locked --no-dev --no-emit-project --output-file runtime-requirements.txt` and verify
first-use bootstrap. Keep the Codex and Claude plugin and marketplace versions synchronized. Never
include credentials or runtime `.hexaharness/` evidence in a commit.

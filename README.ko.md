...
CLI는 내부 실행 계층이며 주 사용자 인터페이스가 아닙니다. 플러그인을 설치한 뒤 `hexa`를 별도로
설치할 필요가 없습니다.
...
- 요구사항을 파악하고 동작, 아키텍처, 범위, 비용 또는 비가역적 결과에 실질적인 영향을 주는
  결정만 질문합니다.
- 저장소의 기존 규칙에 맞춰 설계와 인수 기준을 장기 보존되는 문서로 남깁니다.
...
- 외부 상태를 변경하거나 되돌리기 어려운 작업 직전에 멈춰 승인을 요청합니다.
...
| push, publish, release, deploy 또는 외부 서비스 변경 | 정확한 작업 직전에 승인 요청 |
| 중요한 데이터 삭제, 공유 권한 변경, 메시지 전송 또는 결제 | 정확한 작업 직전에 승인 요청 |
...
승인이 거절되면 에이전트는 준비된 작업을 취소하고 사유를 기록하며 1회용 nonce를 영구 폐기합니다.
외부 작업은 실행되지 않으며 사용자가 내부 상태나 task ID를 정리할 필요도 없습니다.
...
4. **검증** — 자동 실행 가능한 검증 센서를 실행하고 원인을 수정하며 필요한 독립 검토를 수행합니다.
...
| `hexa start` | 복구 가능한 task 체크포인트 생성 |
...
승인이 필요한 작업에서는 에이전트가 내부적으로 `prepare-external`을 사용해 비밀값을 가린 표시,
1회용 nonce, 작업 ID·프로토콜 단계·전체 인자 목록에 결합된 로컬 키 기반 HMAC-SHA-256을
저장합니다. 실행 도중 세션이 끊기면 재조정 상태로 남습니다. 0이 아닌 종료, 시간 초과, 비상
중단처럼 결과가 불확실한 경우도 동일합니다. 에이전트가 외부 시스템을 먼저 확인하므로, 성공 여부를
모른 채 같은 작업을 다시 실행하지 않습니다. 사용자가 이 체크포인트나 결합값을 직접 만들 필요는
없습니다.
